...
#
# Pass through the instructions once, then take
# the signal resulting on wire a, override wire b
# to that signal, and reset the other wires
# (including wire a) and re-run the instructions.
...
if __name__ == '__main__':
   ...
   # Make two passes through the the booklet of
   # instructions. The second time through, modify
   # the instruction assigning wire b to the value
   # found in wire a after the first pass.
   wire_a = 0
   count = 0
   while count < 2:
      count += 1
      ...
      # Make a copy of the booklet instructions
      # which will be modified as progress is
      # made.
      book_copy = [ i for i in booklet ]

      # If this is the second time through, 
      if wire_a != 0:
         # Find the instruction assigning b and
         # reassign the value assigned to it to
         # the value in wire a from the first
         # pass.
         for i in range(len(book_copy)):
            if book_copy[i][-1] == 'b':
               book_copy[i][0] = str(wire_a)
               break

      # Continue to execute instructions as long as
      # there are instructions to execute.
      while len(book_copy) > 0:
         next_round = list()
         for ops in book_copy:
            try:
               # Assignment
               if len(ops) == 3:
                  if ops[0].isnumeric():
         ...
         book_copy = next_round

      # Print the value found on wire a and store
      # the value.
      print('wires[a] = ' + str(wires['a']))
      wire_a = wires['a']
