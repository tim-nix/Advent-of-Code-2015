...
# The cats and trees readings indicates that there
# are greater than that many, while the
# pomeranians and goldfish readings indicate that
# there are fewer than that many. All other
...
if __name__ == '__main__':
   ...
   for sue in sues:
      ...
      for i in range(2, len(sue), 2):
         # Check for 'cats' or 'trees'.
         if (sue[i] == 'cats') or (sue[i] == 'trees'):
            if sue[i+1] > correct_sue[sue[i]]:
               count += 1
         # Check for 'pomeranians' or 'goldfish'.
         elif (sue[i] == 'pomeranians') or (sue[i] == 'goldfish'):
            if sue[i+1] < correct_sue[sue[i]]:
               count += 1
         # Check for all other characteristics.
         elif sue[i+1] == correct_sue[sue[i]]:
            count += 1
