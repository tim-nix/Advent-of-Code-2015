...
# durability, flavor, texture, and calories.
def parseInput(values):
   ...
   for line in values:
      ...
      # starting with 2.
      recipe.append([ int(parts[i]) for i in range(2, len(parts), 2) ])
...
if __name__ == '__main__':
   ...
   # Calories should total this given value.
   total_calories = 500
   ...
   for combo in good_combos:
      ...
      for j in range(len(recipe[0])):
         if j < (len(recipe[0]) - 1):
            ...
            # property score.
            total_score *= score
         # Calculate the calories for this combo.
         else:
            calories = 0
            for i in range(len(combo)):
               calories += combo[i] * recipe[i][j]

      # Retain the highest total score found for
      # cookies with required calorie count.
      if (total_score > best_score) and (calories == total_calories):
